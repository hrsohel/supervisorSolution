...
const multiFiles = () => {
    ...
    const submit = (e) => {
        ...
        if(!serial || !desc || !video || !title) alert("All Feilds Required!")
        else if(video.type.split("/")[0] !== "video") alert("Only Video File Allowed")
        else {
            setLoading(true)
            const formData = new FormData()
            formData.append("ccode", router.query.ccode)
                ...
                <form className='px-2' method='post' encType='maltipart/form-data'>
                    ...
                    <div className='flex items-center justify-between flex-wrap lg:flex-nowrap'>
                        <div className=''>
                            ...
                            <input type="text" name="ccode" id="ccode" className='text-lg px-2 my-2 py-1 w-full outline-none border-2 border-blue-600 rounded-md' value={useRouter().query.ccode}/>
                        </div>
                        <div className=''>
                        ...
                        <div className=''>
                            <label className='font-semibold text-lg' htmlFor="title">Course Title</label>
                            <input onChange={e => setTitle(e.target.value)} type="text" name="title" id="title" className='text-lg px-2 my-2 py-1 w-full outline-none border-2 border-blue-600 rounded-md' />
                        </div>
                        <div className='my-2'>
