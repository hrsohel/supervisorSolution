...
const checkout2 = () => {
    ...
    const carts = data?.message.map(info => {
        return info.cid
    })
    const response = useSWR("/api/getComMem", fetcher2)
    // console.log(response.data?.message[0].courses === carts)
    ...
    React.useEffect(() => {
        (async() => {
            const {data} = await axios.put(`/api/cart/cartTotal`)
        })()
    }, [])
    return (
        <>
            ...
            <div className='container pt-28 mx-auto flex items-start justify-around p-4 flex-col md:flex-row'>
                ...
                <div className='px-4 w-full md:w-1/2'>
                    ...
                    {
                        data?.message.length ? <button onClick={onClickProceed} className='bg-blue-700 text-2xl font-semibold p-4 text-center text-white w-full' disabled={dueCourse > 0 && data?.companyToken || !response.data?.message.length && data?.companyToken}>
                            Proceed
                        </button>
