...
const ComSigUp = ({comShow, setComShow}) => {
    ...
    const submit = async e => {
        e.preventDefault()
        setLoading(true)
        if(!formData.name || !formData.address || !formData.email || !formData.password) alert('All feild required')
        else {
            const {data} = await axios.post('/api/company', formData)
            alert(data.message) 
            if(data.status === 200) history.reload("/companyLogin")
        }
        setLoading(false)
    }
    return (
            ...
            <div className='max-w-7xl lg:py-12 mx-auto  min-h-screen flex justify-center items-center'>
                <div className="lg:card lg:w-2/4 h-full bg-[#e2f5ff] shadow-xl lg:pb-12">
                    ...
                    <div className=' px-2 mt-4'>
                        <h2 className='text-[#1F497B] font-semibold mb- 2'>Name Of Company<span className='text-red-500'>*</span></h2>
                        <div className='flex gap-2'>
                            <input onChange={changeValue} type="text" placeholder="Company" className="input input-bordered input-sm w-full" name='name'/>
                    ...
                    <div className='px-2 mt-2'>
                        ...
                        <div >
                            <input onChange={changeValue} type="email" name='email' placeholder="@gmail.com" className="input input-bordered input-sm w-full " />
                        </div>
                    </div>
                    {/* <div className='px-2 mt-2'>
                        <h2 className='text-[#1F497B] font-semibold mb-2'>Password<span className='text-red-500'>*</span></h2>
                        <div >
                            <input type="password" placeholder="password" className="input input-bordered input-sm w-full " />
                        </div>
                    </div> */}
